...
Date.prototype.yyyymmddhhmm = function (middleChar = ' ', removeTimeOffset = false) {
	let d = new Date(this.valueOf())
	if (removeTimeOffset) {
		d.setMinutes(d.getMinutes() + (new Date()).getTimezoneOffset())
	}

	return `${d.getFullYear()}-${(d.getMonth() + 1).toDigit(2)}-${d.getDate().toDigit(2)}${middleChar}${d.getHours().toDigit(2)}:${d.getMinutes().toDigit(2)}}`
}
...
String.prototype.toConnectorId = function () {
	var val = this.valueOf()
	if(val && val.length==9){
		return `${val.substr(0,3)}.${val.substr(3,3)}.${val.substr(6,3)}`
	}else{
		return val
	}
}
